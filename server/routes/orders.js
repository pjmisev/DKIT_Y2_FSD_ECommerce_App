...
router.get(`/api/orders`, verifyUsersJWTPassword, (req, res, next) =>
{
    ordersModel.find({})
        .then(data =>
        {
...
router.get(`/api/orders/:id`, verifyUsersJWTPassword, (req, res, next) => {
    ordersModel.findById(req.params.id)
        .then(data =>
        {
            console.log(req.decodedToken)
            // if(req.decodedToken._id) return next(createError(403, `User is not the creator of this order.`));
            res.json(data);
        })
        .catch(err => next(createError(500, `A server error has occurred.`)));
})

router.post(`/api/orders`, verifyUsersJWTPassword, (req, res, next) =>
{
...
router.put(`/api/orders/:id`, verifyUsersJWTPassword, (req, res, next) =>
{
    if(
        validateString(req.body.status)
    )
